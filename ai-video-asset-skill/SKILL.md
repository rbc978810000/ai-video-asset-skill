...
`collect-scene-frames` 使用 `--transnet-root`、`--transnet-python`、`--transnet-weights` 或 `AI_VIDEO_TRANSNET_ROOT`、`AI_VIDEO_TRANSNET_PYTHON`、`AI_VIDEO_TRANSNET_WEIGHTS` 指定的 TransNetV2 环境；为兼容原本地布局，若 skill 附近存在 `picVideos/` 也会尝试复用。默认输出仍写入 `00_调研/视频参考帧/`；每个 `场景_XXXX` 只保存一张中点代表图，并复用 `remwm/opencv/none` 去水印后端。
